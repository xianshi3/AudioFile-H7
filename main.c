...
/* 通用函数 */
static void create_fixed_nav_bar(lv_obj_t *parent, const char *title);
static lv_obj_t* create_scrollable_area(lv_obj_t *parent, int y, int height);
...
static void create_fixed_nav_bar(lv_obj_t *parent, const char *title)
{
    ...
    lv_obj_set_style_bg_color(back_btn, COLOR_PRIMARY, 0);
...
static lv_obj_t* create_scrollable_area(lv_obj_t *parent, int y, int height)
{
    ...
    lv_obj_set_size(area, 460, height);
    lv_obj_set_pos(area, 0, y);
    lv_obj_set_style_bg_opa(area, LV_OPA_TRANSP, 0);
    lv_obj_set_style_border_width(area, 0, 0);
    lv_obj_set_style_pad_all(area, 0, 0);
    lv_obj_set_style_pad_top(area, 5, 0);
    lv_obj_set_style_pad_bottom(area, 5, 0);
    ...
    lv_obj_add_style(area, &style_scrollbar, LV_PART_SCROLLBAR);
    
    /* 设置滚动条可见 */
    lv_obj_set_scroll_snap_x(area, LV_SCROLL_SNAP_NONE);
    lv_obj_set_scroll_snap_y(area, LV_SCROLL_SNAP_NONE);
...
static void create_main_screen(void)
{
    ...
    lv_obj_set_scrollbar_mode(btn3, LV_SCROLLBAR_MODE_OFF);
...
static void create_file_manager_screen(void)
{
    ...
    /* 1. 固定的导航栏 */
    create_fixed_nav_bar(app_ctx->app_screen, "File Manager");
    ...
    lv_obj_clear_flag(app_ctx->path_label, LV_OBJ_FLAG_SCROLLABLE);
    lv_label_set_long_mode(app_ctx->path_label, LV_LABEL_LONG_SCROLL_CIRCULAR);
    
    /* 3. 可滚动的文件列表区域 */
    lv_obj_t *scroll_area = create_scrollable_area(app_ctx->app_screen, NAV_BAR_HEIGHT + 30, 370);
    
    /* 文件列表容器 */
    lv_obj_t *list_cont = lv_obj_create(scroll_area);
    lv_obj_set_size(list_cont, 440, 800);  /* 设置一个很大的高度，确保内容可以滚动 */
    ...
    lv_obj_clear_flag(list_cont, LV_OBJ_FLAG_SCROLLABLE);  /* 容器本身不滚动，由父容器滚动 */
    
    /* 文件列表 */
    app_ctx->file_list = lv_list_create(list_cont);
    lv_obj_set_size(app_ctx->file_list, 430, 780);  /* 设置具体大小 */
    lv_obj_set_style_border_width(app_ctx->file_list, 0, 0);
    lv_obj_set_style_bg_color(app_ctx->file_list, COLOR_CARD, 0);
    lv_obj_clear_flag(app_ctx->file_list, LV_OBJ_FLAG_SCROLLABLE);  /* 列表本身不滚动，由父容器滚动 */
...
static void create_audio_player_screen(void)
{
    ...
    /* 1. 固定的导航栏 */
    create_fixed_nav_bar(app_ctx->app_screen, "Audio Player");
    
    /* 2. 可滚动的区域 */
    lv_obj_t *scroll_area = create_scrollable_area(app_ctx->app_screen, NAV_BAR_HEIGHT, 400);
    
    /* 内容容器 - 高度足够大以确保滚动 */
    lv_obj_t *content = lv_obj_create(scroll_area);
    lv_obj_set_size(content, 440, 600);
    lv_obj_set_pos(content, 10, 0);
    lv_obj_set_style_border_width(content, 0, 0);
    lv_obj_set_style_bg_opa(content, LV_OPA_TRANSP, 0);
    lv_obj_clear_flag(content, LV_OBJ_FLAG_SCROLLABLE);
    
    /* 当前播放信息 */
    app_ctx->track_info = lv_label_create(content);
    ...
    lv_obj_clear_flag(prev_icon, LV_OBJ_FLAG_SCROLLABLE);
    ...
    lv_obj_clear_flag(app_ctx->play_btn, LV_OBJ_FLAG_SCROLLABLE);
    ...
    lv_obj_clear_flag(app_ctx->play_icon, LV_OBJ_FLAG_SCROLLABLE);
    ...
    lv_obj_clear_flag(next_icon, LV_OBJ_FLAG_SCROLLABLE);
    ...
    lv_obj_clear_flag(stop_icon, LV_OBJ_FLAG_SCROLLABLE);
    ...
    lv_obj_set_pos(list_title, 0, 130);
    lv_obj_clear_flag(list_title, LV_OBJ_FLAG_SCROLLABLE);
    
    /* 播放列表容器 */
    lv_obj_t *list_cont = lv_obj_create(content);
    lv_obj_set_size(list_cont, 440, 250);
    lv_obj_set_pos(list_cont, 0, 150);
    lv_obj_set_style_border_width(list_cont, 1, 0);
    lv_obj_set_style_border_color(list_cont, lv_color_hex(0xE0E0E0), 0);
    lv_obj_set_style_bg_color(list_cont, COLOR_CARD, 0);
    lv_obj_set_style_radius(list_cont, 4, 0);
    ...
    lv_obj_add_style(list_cont, &style_scrollbar, LV_PART_SCROLLBAR);
...
static void create_audio_processor_screen(void)
{
    ...
    /* 1. 固定的导航栏 */
    create_fixed_nav_bar(app_ctx->app_screen, "Audio Processor");
    ...
    /* 内容容器 */
    lv_obj_t *content = lv_obj_create(scroll_area);
    lv_obj_set_size(content, 440, 500);
    lv_obj_set_pos(content, 10, 0);
    lv_obj_set_style_border_width(content, 0, 0);
    lv_obj_set_style_bg_opa(content, LV_OPA_TRANSP, 0);
    lv_obj_clear_flag(content, LV_OBJ_FLAG_SCROLLABLE);
    ...
    lv_obj_set_pos(hint, 0, 240);
    lv_obj_clear_flag(hint, LV_OBJ_FLAG_SCROLLABLE);
    
    /* ADC状态指示 */
    lv_obj_t *adc_status = lv_label_create(content);
    ...
    lv_obj_set_pos(adc_status, 0, 260);
    lv_obj_clear_flag(adc_status, LV_OBJ_FLAG_SCROLLABLE);
    
    /* I2S状态指示 */
    lv_obj_t *i2s_status = lv_label_create(content);
    ...
    lv_obj_set_pos(i2s_status, 0, 280);
    lv_obj_clear_flag(i2s_status, LV_OBJ_FLAG_SCROLLABLE);
    
    /* 添加额外内容确保滚动 */
    for (int i = 0; i < 5; i++) {
        lv_obj_t *space = lv_label_create(content);
        lv_label_set_text(space, " ");
        lv_obj_set_pos(space, 0, 300 + i * 20);
    }
}
...
static void create_effect_config_screen(int effect_idx)
{
    ...
    lv_obj_set_scrollbar_mode(toggle, LV_SCROLLBAR_MODE_OFF);
    
    /* 2. 可滚动的区域 */
    lv_obj_t *scroll_area = create_scrollable_area(app_ctx->effect_config_screen, NAV_BAR_HEIGHT, 400);
    ...
    lv_obj_set_pos(type_label, 0, 10);
    lv_obj_clear_flag(type_label, LV_OBJ_FLAG_SCROLLABLE);
    
    lv_obj_t *type_cont = lv_obj_create(content);
    lv_obj_set_size(type_cont, 440, 80);
    lv_obj_set_pos(type_cont, 0, 35);
    ...
    lv_obj_set_pos(order_label, 0, 125);
    lv_obj_clear_flag(order_label, LV_OBJ_FLAG_SCROLLABLE);
    ...
    lv_obj_set_pos(order_roller, 0, 150);
    ...
    lv_obj_clear_flag(order_roller, LV_OBJ_FLAG_SCROLLABLE);
    ...
    lv_obj_set_pos(param_label, 100, 125);
    lv_obj_clear_flag(param_label, LV_OBJ_FLAG_SCROLLABLE);
    
    /* 参数滑块容器 */
    lv_obj_t *param_cont = lv_obj_create(content);
    lv_obj_set_size(param_cont, 320, 180);
    lv_obj_set_pos(param_cont, 100, 150);
    ...
    lv_obj_clear_flag(param_cont, LV_OBJ_FLAG_SCROLLABLE);
    lv_obj_set_scrollbar_mode(param_cont, LV_SCROLLBAR_MODE_OFF);
    ...
    for (int i = 0; i < 5; i++) {
        lv_obj_t *space = lv_label_create(content);
        lv_label_set_text(space, " ");
        lv_obj_set_pos(space, 0, 350 + i * 20);
    }
}
